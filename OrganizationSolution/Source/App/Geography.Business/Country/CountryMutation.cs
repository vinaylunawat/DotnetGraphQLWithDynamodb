...
namespace Geography.Business.Country
{
    public class CountryMutation : ITopLevelMutation
    {
        ...
        public CountryMutation(ICountryRepository countryRepository, IMapper mapper)
        {
        ...
        private async Task<CountryModel> ResolveCreateCountry(IResolveFieldContext<object> context)
        {
            ...
            var result = _mapper.Map<CountryModel>(addedCountry);
            return result;

        }

        private async Task<CountryModel> ResolveUpdateCountry(IResolveFieldContext<object> context)
        {

            var countryUpdateModel = context.GetArgument<CountryUpdateModel>("country");
            var dbEntity = await _countryRepository.GetByKey<Guid>(countryUpdateModel.Id, default).ConfigureAwait(false);
            if (dbEntity == null)
            {
            ...
            await _countryRepository.UpdateAsync(dbCountry, default).ConfigureAwait(false);
            return _mapper.Map<CountryModel>(countryUpdateModel);
        ...
        private async Task<object> ResolveDeleteCountry(IResolveFieldContext<object> context)
        {
                ...
                var dbEntity = await _countryRepository.GetByKey<Guid>(countryId, default).ConfigureAwait(false);
                ...
                await _countryRepository.DeleteAsync<Guid>(countryId, default).ConfigureAwait(false);
